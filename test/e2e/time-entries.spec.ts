...
describeTimeEntries('time-entries API integration', async () => {
  ...
  async function getLatest(jar: CookieJar): Promise<Response> {
    return fetch(url('/api/time-entries/latest'), { headers: { cookie: jar.header() } });
  }
  ...
  it('GET /api/time-entries/latest returns the newest startedAt', async () => {
    ...
    const newerStop = new Date(Date.now() - 30_000).toISOString();

    await startEntry(jar, token, {
      title: 'Latest Older ' + Date.now(),
      startedAt: olderStart,
      stoppedAt: olderStop,
    });
    const newer = await (
      await startEntry(jar, token, {
        title: 'Latest Newer ' + Date.now(),
        startedAt: newerStart,
        stoppedAt: newerStop,
      })
    ).json();

    const res = await getLatest(jar);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ startedAt: newer.startedAt });
  });

  it('GET /api/time-entries/latest includes a running entry as newest', async () => {
    const { jar, token } = await loginAs('talice@example.com', 'secret');
    const olderStart = new Date(Date.now() - 2 * 3_600_000).toISOString();
    const olderStop = new Date(Date.now() - 3_600_000).toISOString();
    await startEntry(jar, token, {
      title: 'Latest Stopped ' + Date.now(),
      startedAt: olderStart,
      stoppedAt: olderStop,
    });
    const running = await (
      await startEntry(jar, token, { title: 'Latest Running ' + Date.now() })
    ).json();

    const res = await getLatest(jar);
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ startedAt: running.startedAt });

    await patchEntry(jar, token, running.id, { stoppedAt: new Date().toISOString() });
  });

  it('GET /api/time-entries/latest returns null when the user has no entries', async () => {
    const { jar } = await loginAs('tcarol@example.com', 'secret');
    const res = await getLatest(jar);
    expect(res.status).toBe(200);
    expect(await res.json()).toBeNull();
  });

  it('GET /api/time-entries/latest ignores another user newer entry', async () => {
    const alice = await loginAs('talice@example.com', 'secret');
    const bob = await loginAs('tbob@example.com', 'secret');

    // Make Alice's newest deliberately older than Bob's newest.
    const aliceStart = new Date(Date.now() - 5 * 60_000).toISOString();
    const aliceStop = new Date(Date.now() - 4 * 60_000).toISOString();
    const aliceEntry = await (
      await startEntry(alice.jar, alice.token, {
        title: 'Alice Latest Scope ' + Date.now(),
        startedAt: aliceStart,
        stoppedAt: aliceStop,
      })
    ).json();

    const bobStart = new Date(Date.now() - 30_000).toISOString();
    const bobStop = new Date(Date.now() - 10_000).toISOString();
    const bobEntry = await (
      await startEntry(bob.jar, bob.token, {
        title: 'Bob Newer Latest ' + Date.now(),
        startedAt: bobStart,
        stoppedAt: bobStop,
      })
    ).json();

    const res = await getLatest(alice.jar);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.startedAt).not.toBe(bobEntry.startedAt);
    // Alice's own newest is at least the one just created (may be newer if suite left one).
    expect(new Date(body.startedAt).getTime()).toBeGreaterThanOrEqual(
      new Date(aliceEntry.startedAt).getTime(),
    );
  });

  it('GET /api/time-entries/latest rejects unauthenticated requests', async () => {
    const res = await fetch(url('/api/time-entries/latest'));

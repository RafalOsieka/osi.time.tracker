...
import { RemoteAdapterError, type RemoteTimeLogDto } from '@osi/remote-trackers/contracts';
...
describe('useRemoteSyncClient', () => {
  ...
  it('never resolves the current account to fetch logs (REQ-333)', async () => {
    const client = useRemoteSyncClient(config);
    await client.fetchTimeLogs({ spentOn: '2026-03-15', workPackageIds: ['42'] });
    await client.fetchTimeLogsInRange({ from: '2026-08-01', to: '2026-08-31' });
    expect(getCurrentAccount).not.toHaveBeenCalled();
    expect(fetchTimeLogs).toHaveBeenCalledWith({
      spentOn: '2026-03-15',
      workPackageIds: ['42'],
    });
    expect(fetchTimeLogsInRange).toHaveBeenCalledWith({ from: '2026-08-01', to: '2026-08-31' });
  });

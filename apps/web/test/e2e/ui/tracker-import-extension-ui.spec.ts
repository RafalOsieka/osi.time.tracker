...
describeTrackerImportExtensionUi('tracker remote-log import UI flow (extension mode)', async () => {
  ...
  it('repeats the import journey through the approved extension and reaches the same result', async () => {
    ...
    // `createPage('/')` would navigate before `installExtension` registers its
    // `addInitScript`, and this app's post-login redirect is a client-side
    // `navigateTo()` (no full reload) — so the init script would never get a
    // document to run on. Create the page bare, install first, navigate second.

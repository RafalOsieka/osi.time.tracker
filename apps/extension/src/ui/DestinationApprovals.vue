...
const { websites, destinations, website, provider, destinationUrl, httpWarning } = defineProps<{
...
<template>
  <section class="panel" aria-labelledby="destinations-title">
    ...
    <form class="form" @submit="onAdd">
      ...
      <button class="button" data-testid="add-destination" type="submit">
    ...
    <ul v-else class="list">
      <li v-for="item in destinations" :key="destinationKey(item)" class="list-item">
        <span>{{ item.provider }} {{ item.origin }}{{ item.basePath }}</span>
        <button
          class="button-link"
          type="button"
          :data-testid="`revoke-destination-${item.origin}`"
          :aria-label="`${t('approvals.revokeDestination')} ${item.origin}`"

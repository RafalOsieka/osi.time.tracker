...
| 5.2 | Adapter execution mode: extension (desktop) | 🔴       | Desktop browser extension mediates tracker requests when CORS would block direct access. Unavailable on mobile; no fallback to client. Credentials stay browser-held and never enter OSI APIs                 |
| 5.3 | Adapter execution mode: client (browser)    | 🔴       | API calls made directly from the browser. Requires device reachability (including VPN) and tracker CORS for the OSI origin. Credentials are entered and held only in the browser; never persisted server-side |

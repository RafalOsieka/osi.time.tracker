...
| #        | Feature                                               | Priority | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------- | ----------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5.8      | Remote Sync page (day-level push)                     | 🔴       | Day-scoped page opened from the Timer view listing that day's pushable tasks (tasks with entries that day whose Project → Client has a RemoteSystemConfig), each with a rounded aggregate time and linked remote issue; a single action pushes the whole day. Review page (`GET /api/sync/day`, `app/pages/sync/[date].vue`, per-row state/reason, inline linking) implemented via `remote-sync-page`; the day-level push action itself is not yet built |
| 5.9      | Rounding rule applied on push                         | 🔴       | Configured per RemoteSystemConfig; e.g. round up to nearest 15 min. Applied once to each task's summed daily duration before push (one remote log per task/day). Rounding utility (`applyRoundingRule`) and its editable, pre-filled preview on the Remote Sync page landed via `remote-sync-page`; applying it to an actual push is pending 5.10/5.11                                                                                                   |
...
| 5.12     | Set required remote fields on the Remote Sync page    | 🔴       | Adapter-provided options for autocomplete where available, plain input otherwise; a task's previously used value is pre-filled where possible. OpenProject activity options fetch (`buildTimeEntryActivitiesRequest`) and the per-row activity `Select` (pre-selected from `requiredFieldDefaults`) landed via `remote-sync-page`; selections are page-local until push is implemented                                                                   |

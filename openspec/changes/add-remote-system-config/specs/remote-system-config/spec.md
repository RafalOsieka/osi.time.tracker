...
A user SHALL be able to edit and remove the remote configuration of a Client they own.
...
- **THEN** the system SHALL persist the updated values and return the updated configuration

#### Scenario: Remove a configuration
- **WHEN** a user removes the remote configuration of their Client
- **THEN** the system SHALL delete the stored configuration and the client SHALL clear the browser-held secret associated with that configuration id

...
- **Fix:** `no-conditional-empty-object-spread`, `no-known-value-widening`, `no-unknown-returns`, `no-reflect-get` (lazy `db` Proxy), `no-shape-in-symbol-names`; reduce `no-chained-type-assertions` as far as typed fakes allow.
...
- `api-endpoint-conventions`: `{ messageKey, params }` params type is `MessageParams`.

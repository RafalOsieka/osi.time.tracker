...
- [x] 4.2 `test/e2e/ui/extension-mode-ui.spec.ts`: seed a Redmine extension tracker with 6 linked tasks on one day; assert no row shows the activity-error badge, `getActivityOptions` = 1, `fetchTimeLogs` = 1, `getCurrentAccount` = 0, and no operation ever hit the extension's in-flight limit. (The export-then-refresh addition was dropped after two CI attempts each failed a different way — `creates` stayed 0 even for a single task, with no clear cause visible from CI logs alone and no live browser available here to debug further. Guessing a third fix blind isn't worth it: the export-refresh behavior this was meant to add is already covered deterministically at the nuxt-page level in 3.5, and REQ-331 itself — no limit error, correct scope-sharing, no account call — is fully asserted here and passed twice.)

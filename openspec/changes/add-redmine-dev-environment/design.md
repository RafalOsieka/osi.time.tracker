...
- Documented bring-up/teardown and the one-time manual steps (password change, REST API enablement, sample project/issues, API access key).
...
- [REST API disabled by default] → documented admin toggle; without it every adapter call fails — called out prominently in `AGENTS.md`.

...
6. **Skip-build env `NUXT_TEST_SKIP_BUILD=1`.** Global setup for api/ui: if set and `.output` exists → no `pnpm build`; if set and missing → fail. Db project's global setup never builds. CI `build` job uploads `.output` (retention 1 day); api/ui download to the same path. Build job sets `NUXT_BUILD_SOURCEMAP=1` and `IS_E2E=true` so the artifact has maps **and** the e2e login/global rate-limit knobs (nuxt-security `routeRules` are compiled in; runtime `IS_E2E` in api/ui is too late). Production Dockerfile sets neither. *Alternative: rebuild in each job* — rejected: doubles wall time. *Alternative: always-on production sourcemaps* — rejected: leaks sources in the self-hosted image.

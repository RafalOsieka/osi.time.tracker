...
- Each project DTO carries the owning client's `clientName`, resolved so it remains visible in the table and edit dialog even after the client is soft-deleted.
...
- **`clientName` resolved on the server, join not filtered by client `deletedAt`.** List/create/update responses include the owning client's `clientName` (list via `LEFT JOIN clients`; create/update via a direct lookup), and this resolution deliberately does **not** filter on the client's `deletedAt`, so a project whose client was later soft-deleted still carries its client's name. The UI renders `clientName` directly from the DTO rather than looking it up in the active-client list (which excludes soft-deleted clients), and the edit dialog seeds its Client select with the project's `clientId`/`clientName` when that client is missing from the active list. _Alternative:_ derive the name client-side from the loaded active-client list — rejected because it renders blank for soft-deleted clients. _Alternative:_ denormalize the client name onto the project row — rejected as premature and prone to drift.

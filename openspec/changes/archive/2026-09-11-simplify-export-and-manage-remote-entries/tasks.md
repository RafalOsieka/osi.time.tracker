...
- [x] 1.1 Add required tracker identity and workspace-scoped remote-log uniqueness to export provenance,
...
- [x] 1.2 Update sync-day and monthly-report DTOs/queries to expose and match tracker-scoped provenance,
  and verify backend unit/e2e tests cover user isolation and App/Direct classification

## 2. Provider-Neutral Deletion

- [x] 2.1 Extend remote-tracker contracts and provider implementations with typed deletion outcomes for
  OpenProject and Redmine, and verify package unit tests cover deleted, not-found, rejected, and unknown
  outcomes
- [x] 2.2 Extend client and browser-extension execution adapters/protocol validation for deletion without
...
- [x] 3.1 Add shared schemas and an authenticated endpoint that links an eligible remote entry to all
...
- [x] 3.2 Add an authenticated endpoint that removes owned provenance after browser-confirmed deletion or
  not-found, and verify API e2e tests cover atomic cascade cleanup, idempotency, and foreign/stale requests
- [x] 3.3 Update monthly aggregation for provenance added by links and removed after deletion, and verify
...
- [x] 4.1 Replace the three-phase export dialog with a compact task-title/duration confirmation and total,
...
- [x] 4.2 Remove per-task report/retry and duration-based possible-duplicate UI/state, and verify existing
  export composable/component unit tests assert failed tasks remain normally actionable

## 5. Reconciliation Frontend

- [x] 5.1 Render same-day remote logs as explicitly Linked or Unlinked and add translated accessible link
...
- [x] 5.2 Add translated accessible delete confirmation for Linked entries, execute remote deletion before
...
- [x] 5.3 Add Playwright journeys for compact export, linking an unlinked entry, and deleting a linked entry
  through mocked client and extension transports, verifying tracker calls, refreshed states, and toasts

## 6. Integration Quality

- [x] 6.1 Keep English and Polish catalogs in parity and verify lint catches no raw user-facing text or
  accessibility regressions in confirmation and row actions
- [x] 6.2 Run `pnpm lint`, `pnpm format:check`, `pnpm type-check`, `pnpm test:unit`, `pnpm test:nuxt`, and the

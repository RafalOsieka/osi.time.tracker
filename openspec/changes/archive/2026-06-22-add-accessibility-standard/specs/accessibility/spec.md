...
## Open Questions
- [ANSWERED] Whether to additionally adopt a runtime axe-core test gate (e.g. `vitest-axe` in the `nuxt` project) — tracked as a deferred follow-up change, not implemented here.
  - ANSWER: No, the axe-core gate is not mandatory. It can be put to backlog (wbs.md)
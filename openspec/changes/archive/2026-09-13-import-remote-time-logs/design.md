...
sortedLogs = logs ordered by numeric remoteLogId ascending
lastEntryStart = cursor + sum(durations of sortedLogs) - lastLog.duration
if lastEntryStart >= dayEnd:  cursor = max(dayStart, that same max(stoppedAt))
for log in sortedLogs:
    startedAt = cursor; stoppedAt = cursor + duration; cursor = stoppedAt
```

The invariant ("after whatever is already on that day, whoever created it") makes two trackers imported in either order, and the overlap month with real local entries, come out non-overlapping. Spill past midnight is fine — entries bucket on `startedAt`'s local day (REQ-291, REQ-150). The fallback check is on the **last entry's own start**, not the cumulative total: a single entry, however long, keeps the `08:00` anchor and simply ends after midnight; the anchor drops to `00:00` only when a *later* entry in the sequence would otherwise start on the next day (an earlier draft checked the cumulative total against `dayEnd` instead, which — with an 8 h anchor on a ~24 h day — fires for any total over 16 h and so could never distinguish "spills past midnight" from "falls back to midnight"; caught while writing the placement function's unit tests). More than 24 h on one day is bad data and may overlap. Running entries are ignored (null `stoppedAt`). The rule is a pure function (`placeImportedEntries`) unit-tested with the two-tracker example. Alternatives — fixed `08:00` for every entry (overlaps), or spreading across the day proportionally (arbitrary and still overlaps across runs) — rejected.
